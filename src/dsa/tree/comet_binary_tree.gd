...
##
## If this succeeds, this returns an Ok, holding the newly made node.
##
## If a node with an identical value according to `eq_fn` already exists, this returns an Err, holding the existing node.
## In the above case, if `overwrite_for_same` was true, this will forcibly overwrite the incoming value to the node.
...
func is_bst(from: CometBinaryTree = self) -> bool:
	if from._l:
		if _comp_fn.call(from._l._v, from._v):
			if not is_bst(from._l): return false
		else: return false
	if from._r:
		if _comp_fn.call(from._v, from._r._v):
			if not is_bst(from._r): return false
		else: return false
	return true
...
func reorder_as_bst(from: CometBinaryTree = self, start: int = 0, end: int = INT64_MIN, nodes: Array[CometBinaryTree] = []) -> CometBinaryTree:
	...
	root.mut_l(reorder_as_bst(from, start, mid - 1, nodes), LinkAndOrphanMode.DIRECT_THEN_NONE)
	root.mut_r(reorder_as_bst(from, mid + 1, end, nodes), LinkAndOrphanMode.DIRECT_THEN_NONE)
	return root
