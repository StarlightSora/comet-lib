...
const INVALID_T2D: Transform2D = Transform2D(NAN, Vector2(NAN, NAN))
...
static func maybe_infinite_t3d_to_optional(to_check: Transform3D) -> OptionalType:
...
static func maybe_infinite_t2d_to_optional(to_check: Transform2D) -> OptionalType:
	if to_check.is_finite():
		return OptionalType.new(to_check)
	else:
		return OptionalType.new(null)

static var _btsd := func(lhs: KVPair, rhs: KVPair) -> bool:
	return lhs.k() < rhs.k()
...
static func blend_transform3ds(
...
## (Dictionary<T, Transform2D>, Dictionary<T, float>, Transform2D?, BTBlendMode?) -> Transform2D
##
## `blend_mode` defaults to `BLEND_WITH_ORIGIN_IF_WEIGHT_UNDER_ONE`.
## Negative weights are not supported; such entries will be ignored.
## The blending is *not* mathematically proven to be truly linear with more than two entries.
##
## NOTE: The return value will be an invalid Transform2D if `blend_mode = NEVER_USE_ORIGIN` and the total weight of the `weights` array is zero.
## To safely guard against this possibility, wrap the return value in a `maybe_infinite_t2d_to_optional` call to convert it to an `OptionalType<Transform2D>`.
##
## Has O(n) time complexity.
static func blend_transform2ds(
	targets: Dictionary[Variant, Transform2D],
	weights: Dictionary[Variant, float],
	origin: Transform2D = Transform2D.IDENTITY,
	blend_mode: BTBlendMode = BTBlendMode.BLEND_WITH_ORIGIN_IF_WEIGHT_UNDER_ONE,
) -> Transform2D:
	var transposed: Array[KVPair] = [] ## Array<KVPair<float, Transform2D>>
	var total_weight: float = 0.0
	var total_entries: int = 0
	for key in weights:
		var value: float = weights[key]
			...
			while transposed.size() > 0:
				...
				if front_1:
					...
					if total_weight <= 1.0 and blend_mode == BTBlendMode.BLEND_WITH_ORIGIN_IF_WEIGHT_UNDER_ONE:
						...
						return front_0.v()
	assert(false, "Unreachable code!")
	return INVALID_T2D if blend_mode == BTBlendMode.NEVER_USE_ORIGIN else origin